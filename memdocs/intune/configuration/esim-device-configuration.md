...
如不確定您的裝置是否支援 eSIM，請連絡您的裝置製造商。 在 Windows 裝置上，您可以確認 eSIM 的支援能力。 如需詳細資訊，請參閱[使用 eSIM 在 Windows 10 電腦上建立行動數據連線](https://support.microsoft.com/help/4020763/windows-10-use-esim-for-cellular-data)。
...
2. 第二個及後面所有資料列都是僅使用一次的唯一啟用代碼，其中包含兩個值：
    ...
    2. 第二個資料行是只以逗號分隔 (結尾沒有逗號) 的比對識別碼。 請參閱下列範例：
        ...
        :::image type="content" source="./media/esim-device-configuration/url-activation-code-examples.png" alt-text="電信業者啟用代碼範例 csv 檔案。":::
...
3. CSV 檔案名稱會變成 Endpoint Manager 系統管理中心中的行動數據訂用帳戶集區名稱。 在上圖中，檔案名稱為 `UnlimitedDataSkynet.csv`。 因此，Intune 會將訂用帳戶集區命名為 `UnlimitedDataSkynet.csv`：
    ...
    :::image type="content" source="./media/esim-device-configuration/subscription-pool-name-csv-file.png" alt-text="行動數據訂閱集區以啟用代碼範例 csv 檔案名稱命名。":::
...
4. 選擇以 [包含] 群組或 [排除] 群組，然後選取群組。
    ...
    :::image type="content" source="./media/esim-device-configuration/include-exclude-groups.png" alt-text="包含裝置群組以在 Microsoft Intune 中指派設定檔。":::
...
3. eSIM 設定檔隨即列出：
    ...
    :::image type="content" source="./media/esim-device-configuration/device-settings-cellular-profiles.png" alt-text="檢視您裝置設定中的 eSIM 設定檔。":::
