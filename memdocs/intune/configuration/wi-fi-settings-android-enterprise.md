...
- **EAP 類型**：選擇用來驗證安全無線連線的可延伸的驗證通訊協定 (EAP) 類型。 選項包括：
  ...
  - **EAP-TLS**：另請輸入：
    ...
    - **用戶端驗證** - **進行用戶端驗證的用戶端憑證 (身分識別憑證)** ：選擇也會部署到裝置的 SCEP 用戶端憑證設定檔。 此憑證是裝置提供給伺服器以驗證連線的身分識別。
...
- **Proxy 設定**：指定您組織所使用的 Proxy 設定。 選項包括：

  - **無** - 您不使用 Proxy 伺服器。
  - **自動** - 選取此選項來使 [Proxy 伺服器 URL]  設定可用；您會使用此設定來指定 Proxy 伺服器，或是包含您 Proxy 伺服器清單的 Proxy 自動設定 (PAC) 檔案。

- **Proxy 伺服器 URL**：在您將 [Proxy 設定]  設定為 [自動]  時即可使用此設定。 指定下列其中一個選項來將裝置導向到您的 Proxy 伺服器：
