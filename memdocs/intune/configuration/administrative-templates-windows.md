...
description: 在 Microsoft Intune 和端點管理員中使用系統管理範本，為 Windows 10 裝置建立設定群組。 您可以使用裝置組態設定檔中的這些設定，控制 Office 程式和 Microsoft Edge、保護 Internet Explorer、存取 OneDrive、使用遠端桌面功能、啟用 [自動播放]、設定電源管理設定、使用 HTTP 列印、控制使用者登入，以及變更事件記錄檔大小。
keywords: ''
...
您可以在 Microsoft Intune 中使用 [系統管理範本] 來完成這項工作。 系統管理範本包含數千項設定，可控制 Microsoft Edge 77 版與更新版本、Internet Explorer、Microsoft Office 程式、遠端桌面、OneDrive 中的功能，以及密碼、PIN 等功能。 這些設定可讓群組管理員使用雲端來管理群組原則。
...
7. 在 [組態設定] 中，選取 [所有設定]，查看依字母順序排列的所有設定清單。 或者，分別設定適用於裝置的設定 ([電腦設定])，以及適用於使用者的設定 ([使用者設定])：
...
8. 當您選取 [所有設定] 時，系統即會列出每項設定。 向下捲動，以使用向前和向後箭號來查看更多設定：

    > [!div class="mx-imgBorder"]
    > ![查看設定清單範例並使用上一個和下一個按鈕](./media/administrative-templates-windows/administrative-templates-sample-settings-list.png)

9. 選取任何設定。 例如，篩選 **Office**，然後選取 [啟用限制瀏覽]。 設定的詳細描述會隨即顯示。 選擇 [已啟用]、[已停用]，或將設定保留為 [尚未設定] \(預設\)。 此詳細描述也會說明當您選擇 [已啟用]、[已停用] 或 [尚未設定] 時所發生的情況。

    > [!TIP]
    > Intune 中的 Windows 設定會與您在本機群組原則編輯器 (`gpedit`) 中看到的內部部署群組原則路徑相互關聯

10. 當您選取 [電腦設定] 或 [使用者設定]時，系統即會顯示設定類別。 您可以選取任意類別來查看可用設定。
...
這些範本提供數千項設定。 若要更輕鬆地尋找特定設定，請使用內建功能：

- 在您的範本中，選取 [設定]、[狀態]、[設定類型] 或 [路徑] 欄來排序清單。 例如，選取 [路徑] 資料行，並使用下一步箭頭來查看 `Microsoft Excel` 路徑中的設定。

- 在您的範本中，使用 [搜尋] 方塊來尋找特定設定。 您可以依設定 (或路徑) 搜尋。 例如，選取 [所有設定]，然後搜尋 `copy`。 這會顯示包含 `copy` 的所有設定：
...
- 您也可以只選取 [電腦設定] 或 [使用者設定] 來縮小搜尋範圍。

  例如，若要查看所有可用的 Internet Explorer 使用者設定，請選取 [使用者設定]，然後搜尋 `Internet Explorer`。 系統只會顯示適用於使用者的 IE 設定：

  :::image type="content" source="./media/administrative-templates-windows/show-all-internet-explorer-settings-user-configuration.png" alt-text="在 ADMX 範本中，選取 [使用者設定]，然後在 Microsoft Intune 中搜尋或篩選 Internet Explorer。":::
