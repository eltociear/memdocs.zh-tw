...
- **選取 Kiosk 模式**：選擇 [single app, full-screen Kiosk] \(單一應用程式、全螢幕 Kiosk\)  。
...
- **應用程式類型**：選取應用程式類型。 選項包括：
  ...
  - **新增 Kiosk 瀏覽器**：選取 [Kiosk 瀏覽器設定]  。 這些設定會控制 Kiosk 上的網頁瀏覽器應用程式。 請確定從 Microsoft Store 取得 [Kiosk 瀏覽器應用程式](https://businessstore.microsoft.com/store/details/kiosk-browser/9NGB5S5XG2KP)、將其作為[用戶端應用程式](../apps/apps-add.md)新增至 Intune， 然後將應用程式指派給 Kiosk 裝置。
    ...
    - **允許的網站**：使用此設定可允許開啟特定網站。 換句話說，使用此功能可在裝置上限制或防止網站。 例如，您可以允許開啟位於 `http://contoso.com` 的所有網站。 預設會允許所有網站。
      ...
      若要允許特定網站，請上傳包含不同行上所允許網站清單的檔案。 如果您未新增檔案，則會允許所有網站。 根據預設，Intune 可支援萬用字元。 因此，當輸入網域 (例如 `sharepoint.com`) 時，將自動允許子網域，例如 `contoso.sharepoint.com`、`my.sharepoint.com` 等。
 ...
 - **指定應用程式重新啟動的維護視窗**：預設值為 [未設定]，請選取 [必要] 來檢查需要重新啟動才能完成安裝的應用程式。
 
     如果您使用 Kiosk 瀏覽器或其他商務應用程式的 Microsoft Store，請針對需要重新啟動才能完成更新安裝的應用程式，決定檢查頻率。 如果未設定，則商務應用程式的 Microsoft Store 會在安裝應用程式更新的 3 天後，於未排程的時間重新啟動。
     
     - **維護視窗開始時間**：選取一天中的日期與時間，以開始檢查用戶端是否有任何需要重新啟動的應用程式更新。 預設開始時間是午夜 (或零分鐘)。
     
     - **維護視窗週期**：預設為每天。
         設定應用程式更新維護視窗的執行頻率。 建議每天執行，以避免應用程式於未排程的時間重新啟動。
...
- **選取 Kiosk 模式**：選擇 [多重應用程式 Kiosk]  。

- **以 S 模式的 Windows 10 裝置為目標**：
  - **是**：允許 Kiosk 設定檔中的市集應用程式應用程式和 AUMID 應用程式 (Win32 應用程式除外)。
