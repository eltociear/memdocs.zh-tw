...
- 在 iOS/iPadOS 裝置上，公司入口網站應用程式支援 ASAM。 當公司入口網站應用程式在 ASAM 中時，會將裝置鎖定在公司入口網站應用程式中，直到使用者進行驗證為止。 當使用者登入公司入口網站應用程式時，可以使用其他應用程式與裝置上的主畫面按鈕。 當使用者登出公司入口網站應用程式時，裝置會回復到單一應用程式模式，並鎖定在公司入口網站應用程式。

  若要將公司入口網站應用程式轉換為「登入/登出」應用程式 (啟用 ASAM)，請在這些設定中輸入公司入口網站應用程式名稱 (例如 `Microsoft Intune Company Portal`) 與配套識別碼 (`com.microsoft.CompanyPortal`)。 指派此設定檔之後，您必須開啟公司入口網站應用程式來鎖定應用程式，讓使用者可以登入及登出。
  
  當移除裝置設定檔，且該使用者登出後，裝置就不會鎖定在公司入口網站應用程式。
