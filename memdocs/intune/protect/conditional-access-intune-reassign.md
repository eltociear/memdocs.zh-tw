...
如果您已準備好移至 Azure 入口網站，請遵循此主題中的步驟，以重新指派您先前在 Intune 傳統入口網站中建立的條件式存取原則：
...
4. 在 [指派]  區段下，選擇 [使用者和群組]  ，以新的條件式存取原則為目標。
    ...
    > [!IMPORTANT] 
    > 您在 Azure 入口網站中所做的選擇，應該會對應至您在傳統入口網站中所做的選擇。 例如，如已在 Intune 傳統入口網站中選取所有使用者，請在 Azure 入口網站中選取 [所有使用者]  。 此外，如果您已在 Intune 傳統入口網站中選擇 [豁免群組]  選項，則請一併在 Azure 入口網站中排除這些選取的群組。
...
4. 在 [指派]  區段下，選擇 [使用者和群組]  ，以新的條件式存取原則為目標。
    ...
    > [!IMPORTANT] 
    > 您在 Azure 入口網站中所做的選擇，應該會對應至您在 Azure 入口網站中所做的選擇。 例如，如已在 Intune 傳統入口網站中選取所有使用者，請在 Azure 入口網站中選取 [所有使用者]  。 此外，如果您已在 Intune 傳統入口網站中選擇 [豁免群組]  選項，則請一併在 Azure 入口網站中排除這些選取的群組。
...
8. 在 [條件]  刀鋒視窗中，選擇 [用戶端應用程式]  ，然後選擇適用的用戶端應用程式。 如果已選擇封鎖 Intune 不支援的用戶端，請使用 [僅將原則套用至支援的平台]  選項。
