...
4. 在 [新增合約詳細資料]  區域中，指定下列資料：
    ...
    - **發行者** (必要)： 當您開始輸入發行者名稱時，服務會擷取內含您所輸入字母的所有發行者名稱。 例如，如果您輸入 "soft"，服務便會擷取名稱中包含 "soft" 的所有發行者名稱，例如 "Microsoft" 和 "Microsoft Research"。 發行者名稱是從「軟體資產類別」擷取。 您必須先選取發行者，才能輸入產品標題。
