...
2. 在 [應用程式套件檔案] 窗格中，選取 [瀏覽] 按鈕。 然後選取副檔名為 **.msi**、 **.appx** 或 **.appxbundle** 的 Windows 安裝檔案。
   應用程式詳細資料隨即顯示。
    ...
    > [!NOTE]
    > Windows 應用程式的副檔名包含 **.msi**、 **.appx**、 **.appxbundle**、 **.msix**、 和 **.msixbundle**。 如需有關 **.msix** 的詳細資訊，請參閱 [MSIX 文件](https://docs.microsoft.com/windows/msix/)與 [MSIX 應用程式散發](https://docs.microsoft.com/windows/msix/desktop/managing-your-msix-deployment-enterprise)。
