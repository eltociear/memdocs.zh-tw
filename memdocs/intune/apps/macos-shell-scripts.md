...
ms.locfileid: "83990595"
---
# <a name="use-shell-scripts-on-macos-devices-in-intune"></a>在 Intune 中於 macOS 裝置使用 Shell 指令碼
...
6. 選取 [指派] > [選取要包含的群組]。 Azure AD 群組的現有清單會隨即顯示。 選取要接收指令碼的一或多個使用者或裝置群組。 選擇 [選取]。 選擇的群組會顯示在清單中，且會接收指令碼原則。
   > [!NOTE]
   > - 指派給使用者群組的 Shell 指令碼會套用至任何登入 Mac 的使用者。  
   > - 更新 Shell 指令碼的指派，也會更新[適用於 macOS 的 Microsoft Intune MDM 代理程式](macos-shell-scripts.md#microsoft-intune-management-agent-for-macos)指派。
...
可能原因如下：
* 代理程式可能需要簽入，才能接收新的或已更新指令碼。 此簽入程序會每 8 小時發生一次，且與 MDM 簽入不同。 請確定裝置處於喚醒狀態，且已連線到網路，以順利完成代理程式簽入，並等候代理程式簽入。 您也可以要求終端使用者在 Mac 上開啟公司入口網站，選取裝置，並按一下 [檢查設定]。
...
當指令碼執行完成時，就會將指令碼執行狀態回報給 Microsoft Endpoint Manager 系統管理主控台。 如果指令碼排定以設定的頻率定期執行，則只會在第一次執行時回報狀態。
...
在已安裝代理程式的受控 Mac 上，開啟**公司入口網站**，選取本機裝置，然後按一下 [檢查設定]。 這樣做會隨即起始 MDM 簽入，以及代理程式簽入。

或者，您可以開啟**終端機**，執行 `sudo killall IntuneMdmAgent` 命令以終止 `IntuneMdmAgent` 處理序。 `IntuneMdmAgent` 處理序會立即重新啟動，這將會起始 Intune 的簽入。

> [!NOTE]
> 在 Microsoft 端點管理員管理主控台中，裝置的**同步處理**動作會起始 MDM 簽入，而不會強制代理程式簽入。
 ...
 ### <a name="why-are-scripts-running-even-though-the-mac-is-no-longer-managed"></a>為什麼即使 Mac 不再受管理，仍會持續執行指令碼？
 當指派了指令碼的 Mac 不再受管理時，系統並不會立即移除代理程式。 代理程式會在下一次代理程式簽入 (通常每 8 小時一次) 時偵測到 Mac 已不受管理，才取消已排程的指令碼執行。 因此，只要任何儲存在本機的指令碼排程執行頻率比下一次排程代理程式簽入頻率更高，就會執行這些指令碼。 當代理程式無法簽入時，最多會在 24 小時 (裝置喚醒時間) 內重試簽入，之後從 Mac 中自行移除。
