...
| **上傳品牌影像** | 上傳反映貴組織品牌的影像。<p><ul><li>建議的影像寬度：大於 1125 px (必須至少為 650 px)</li><li>影像大小上限：1.3 MB</li><li>檔案類型：PNG、JPG 或 JPEG</li><li>其顯示於下列位置：</li><ul><li>iOS/iPadOS 公司入口網站：使用者設定檔頁面上的背景影像。</li><li>公司入口網站： 使用者設定檔頁面上的背景影像。</li><li>Android Intune 應用程式：在選單中，以及作為使用者設定檔頁面上的背景影像。</li></ul></ul> |
...
- 避免使用一般內建影像。 此影像應該反映貴組織的品牌，並讓使用者感覺熟悉。 如果您沒有影像，與其使用對使用者不具任何意義的一般影像，不如不要使用影像。
...
輸入貴組織的支援資訊，讓員工可以提出問題。 在終端使用者體驗中，此支援資訊將顯示於 [支援]  、[說明與支援]  及 [技術支援中心]  頁面上。
...
<sup>(1)</sup> **已知問題：** 如果您將應用程式設定為需要註冊才能安裝，同時也將裝置註冊設定為「無法使用」，則 Android 上的公司入口網站應用程式仍將引導使用者進行註冊。 這將在不久後移除。
