---
title: 1710 的診斷資料 | Configuration Manager
titleSuffix: Configuration Manager
description: 了解 Configuration Manager 1710 版所收集的診斷及使用資料層級。
ms.date: 11/20/2017
...
ms.locfileid: "81697036"
---
# <a name="levels-of-diagnostic-usage-data-collection-for-version-1710-of-configuration-manager"></a>Configuration Manager 1710 版的診斷使用資料收集層級

適用於：  Configuration Manager (最新分支)

Configuration Manager 1710 版會收集三個層級的診斷及使用方式資料：[基本]  、[增強]  和 [完整]  。 這項功能預設會設定為增強層級。 下列各節提供有關每個層級所收集之資料的詳細說明。
...
針對 Configuration Manager 1710 版，這個層級包括下列資料：
...
- 基本站台系統伺服器資訊 (使用的站台系統角色、網際網路和 SSL 狀態、作業系統、處理器、實體或虛擬機器，以及站台伺服器高可用性的使用方式)
...
- ***[更新]*** Configuration Manager 用戶端版本、作業系統版本以及 Office 版本的計數
...
- **應用程式管理：**  
   ...
   - 商務用 Microsoft Store 應用程式計數和同步統計資料 (包括應用程式的摘要類型、已授權應用程式狀態，以及線上和離線已授權應用程式數目)  
...
- **雲端服務：**
  ...
  - 雲端管理閘道的設定與使用方式統計資料，包括區域和環境的計數，以及驗證/授權統計資料
...
- ***[新增]*** 共同管理
  - ***[新增]*** 彙總的共同管理使用方式統計資料，包括已註冊的用戶端、接收原則的用戶端，工作負載狀態、試驗/排除集合大小、註冊錯誤的數量
  - ***[新增]*** 共同管理註冊方法的用戶端計數
  - ***[新增]*** 共同管理註冊的錯誤統計資料
  - ***[新增]*** 註冊排程和歷程記錄統計資料
  - ***[新增]*** 符合共同管理資格的用戶端計數
...
- **Endpoint Protection：**  
   ...
   - ***[新增]*** Windows Defender 惡意探索防護原則、部署和目標用戶端的計數
