...
一般而言，您要將內容發佈到發佈點，才能讓用戶端電腦使用該內容。 (在您針對特定部署使用隨選內容發佈時，會是例外狀況。)當您發佈內容時，Configuration Manager 會將內容檔案儲存在套件中，然後將套件發佈到發佈點。 套件的內容會從站台伺服器的內容庫中提取。 您可以發佈的內容類型包括：  
...
- 系統會從原始套件來源位置，將內容檔案複製到擁有該套件內容來源之站台上的內容庫
- 封裝版本會遞增  
