...
| **允許在 Windows 診斷資料中傳送裝置名稱** | AllowDeviceNameInTelemetry | 如果您選擇在 Configuration Manager 中傳送裝置名稱，則可以透過將此原則設定為 [停用] 來加以覆寫。 當您停用此設定時，裝置名稱會在電腦分析中顯示為「未知」。 如需詳細資訊，請參閱[裝置名稱](enroll-devices.md#device-name)。 |
