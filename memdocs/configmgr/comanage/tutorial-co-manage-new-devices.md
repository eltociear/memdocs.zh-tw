...
8. 選取 [所有服務] > [Azure Active Directory] > [應用程式註冊]，然後：
   ...
   2. 前往 [API 權限] > 選取 [授與管理員同意] <your tenant>，然後選取 [是]。  

   3. 選取您建立的「原生用戶端」應用程式。

   4. 前往 [API 權限] > 選取 [授與管理員同意] <your tenant>，然後選取 [是]。
...
1. 從主要站台伺服器登入 [Microsoft 端點管理員系統管理中心](https://endpoint.microsoft.com)，並前往 [應用程式] > [所有應用程式] > [新增]。

2. 在 [其他] 下選取 [企業營運應用程式] 作為應用程式類型。

3. 針對 [應用程式套件檔案]，瀏覽至 Configuration Manager 檔案 **ccmsetup.msi** 的位置，然後選取 [開啟] > [確定]。
...
1. 登入 [Microsoft Endpoint Manager 系統管理中心](https://endpoint.microsoft.com)。 選取 [應用程式] > [所有應用程式]，然後選取 [Configuration Manager 用戶端安裝程式啟動程序]，此為所建立用來部署 Configuration Manager 用戶端的應用程式。  

2. 按一下 [屬性]，然後為 [指派] 進行 [編輯]。 在 [必要] 指派下選取 [新增群組]，來設定含有要參與共同管理的使用者與裝置其 Azure Active Directory (AD) 群組。  

3. 依序選取 [檢閱並儲存] 和 [儲存] 來儲存設定。
