...
- 不應與桌面互動。 命令必須以無訊息模式或自動模式執行。  

- 絕不能自行初始化重新啟動。 命令必須使用標準重新啟動代碼 3010 來要求重新啟動。 此行為可確保工作順序步驟會正確處理重新啟動。 如果命令確實傳回 3010 結束代碼，工作順序引擎就會重新啟動電腦。 重新啟動之後，工作順序會自動繼續進行。
...
- 不應與桌面互動。 必須以無訊息模式或自動模式來執行指令碼。  

- 絕不能自行初始化重新啟動。 指令碼必須使用標準重新啟動代碼 3010 來要求重新啟動。 此行為可確保工作順序步驟會正確處理重新啟動。 如果指令碼確實傳回 3010 結束代碼，工作順序引擎就會重新啟動電腦。 重新啟動之後，工作順序會自動繼續進行。
